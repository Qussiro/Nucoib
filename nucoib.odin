...
draw_char :: proc(c: u8, pos: rl.Vector2, scale: f32, fg_color: rl.Color = rl.WHITE) {
    ...
    rl.DrawTexturePro(font_texture, source, dest, {}, 0, fg_color) 
}
...
input :: proc(dt: f32) {
    ...
    drill: if rl.IsKeyDown(rl.KeyboardKey.D) {
        ...
        if check_boundaries(player_pos+1) {
            ...
            buildings[player.x][player.y] = Drill{capacity = 20}
        }
    }
    ...
    if rl.IsKeyDown(rl.KeyboardKey.C) {
        conveyor, ok := buildings[player.x][player.y].(Conveyor)
        if (ok && conveyor.direction != direction) || buildings[player.x][player.y] == nil do buildings[player.x][player.y] = Conveyor{direction = direction}
    }
...
draw_border :: proc(x, y, w, h: int, bg_color: rl.Color = {}, fg_color: rl.Color = rl.WHITE, fill: bool = false) {
    if fill {
        dest := rl.Rectangle {
            x = f32(x) * char_width * scale,
            y = f32(y) * char_width * scale,
            width = f32(w) * char_width * scale,
            height = f32(h) * char_height * scale,
        }
    ...
    for i := 0; i < w; i += 1 {
        ...
        if (i == 0 || i == w - 1) {
            draw_char('+', upper_pos, scale, fg_color)
            draw_char('+', lower_pos, scale, fg_color)
        } else {
            draw_char('-', upper_pos, scale, fg_color)
            draw_char('-', lower_pos, scale, fg_color)
        }
    ...
    for i := 1; i < h - 1; i += 1 {
        ...
        draw_char('|', left_pos, scale, fg_color)
        draw_char('|', right_pos, scale, fg_color)
    }
}
...
main :: proc() {
    ...
    for !rl.WindowShouldClose() {
            ...
            for j := 0; j < WORLD_HEIGHT; j += 1 {
                switch &building in buildings[i][j] {
                    ...
                    case Drill: 
                        ...
                        next_ore := world[i + building.next_tile % 2][j + building.next_tile / 2]
        ...
        first_col := max(0, player.x - grid_cols/2) + 1
        last_col  := min(player.x + (grid_cols+1)/2 - 1, WORLD_WIDTH) 
        first_row := max(0, player.y - grid_rows/2) + 1
        last_row  := min(player.y + (grid_rows+1)/2 - 1, WORLD_HEIGHT)
                ...
                switch building in buildings[i][j] {
                    ...
                    case Drill: 
                        dest := rl.Rectangle {
                            x = pos.x,
                            y = pos.y,
                            width = char_width * scale * 2,
                            height = char_height * scale * 2,
                        }
                ...
                switch building in buildings[i][j] {
                    ...
                    case Conveyor:
                        switch building.direction {
                            case .Right: draw_char('>', pos, scale)
                            case .Left:  draw_char('<', pos, scale)
        ...
        for i := first_col; i < last_col; i += 1 {
            for j := first_row; j < last_row; j += 1 {
                ...
                if is_conveyor && conveyor.ore_type != .None {
                    ...
                    draw_char(c, ore_offset, scale * ORE_SCALE, rl.GOLD)
                }
            }
        }
        ...
        rl.DrawRectangleV(player_pos, {char_width, char_height}*scale, BG_COLOR)
        draw_char('@', player_pos, scale, rl.VIOLET)
            ...
            switch building in buildings[player.x][player.y] {
                ...
                case Drill:    
                    if len(building.ores) == 0 {
                        text_building = fmt.bprintf(text_buffer[len(text_ore):], "Drill[%v:%v]", nil, nil)
                    } else {
                        text_building = fmt.bprintf(text_buffer[len(text_ore):], "Drill[%v:%v]", building.ores[0].type,building.ores[0].count)
                    }
                    
                case Part:     
                    drill := buildings[building.main_pos.x][building.main_pos.y].(Drill)
                    
                    if len(drill.ores) == 0 {
                        text_building = fmt.bprintf(text_buffer[len(text_ore):], "Drill[%v:%v]", nil, nil)
                    } else {
                        text_building = fmt.bprintf(text_buffer[len(text_ore):], "Drill[%v:%v]", drill.ores[0].type, drill.ores[0].count)
                    }
